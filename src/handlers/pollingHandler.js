...
module.exports = {
    ...
    handlers: async function (rustplus, client, info, mapMarkers, teamInfo, time) {
        try {
            ...
            // Storage monitor scans can be slow on large setups (many monitors / slow RPCs).
            // Run them in the background so they do not stall the main polling loop.
            ...
            // Ensure first successful poll after (re)connect performs an immediate information refresh.
            if (rustplus.isFirstPoll) {
                await withTimeout(
