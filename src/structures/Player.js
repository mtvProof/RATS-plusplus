...
class Player {
    constructor(player, rustplus) {
        ...
        // Load playtime and offline/online times from instance file for this server
        ...
        // Load offline/online timestamps from persistent storage
        if (server && server.playerTimestamps && server.playerTimestamps[this._steamId]) {
            ...
            if (!player.isOnline && timestamps.wentOfflineTime) {
                this._wentOfflineTime = new Date(timestamps.wentOfflineTime);
            }
            if (player.isOnline && timestamps.cameOnlineTime) {
    ...
    persistTimestamps() {
        ...
        if (server) {
            ...
            server.playerTimestamps[this._steamId] = {
                wentOfflineTime: this._wentOfflineTime ? this._wentOfflineTime.toISOString() : null,
                cameOnlineTime: this._cameOnlineTime ? this._cameOnlineTime.toISOString() : null
            };
            Client.client.setInstance(this._rustplus.guildId, instance);
        }
    }
